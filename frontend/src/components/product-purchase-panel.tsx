  ...
  return (
    <aside className="productCartPanel" aria-label={isService ? 'Service booking options' : 'Product cart options'}>
      ...
      <p className="checkoutHint">
        {isService
          ? 'Add this service to cart, then complete checkout or continue browsing.'
          : 'Add this product to cart, continue shopping, or checkout when ready.'}
      </p>
      ...
      <div className="productCartActions">
        <button type="button" className="requestButton" onClick={() => addItem(false)}>
          {isService ? 'Add service' : 'Add to cart'}
        </button>
        <button type="button" className="secondaryButton" onClick={() => addItem(true)}>
          Checkout now
        </button>
      </div>
      ...
      <div className="mobileProductBar">
        ...
        <button type="button" onClick={() => addItem(false)}>{isService ? 'Add service' : 'Add to cart'}</button>
        <button type="button" onClick={() => addItem(true)}>Checkout</button>
      </div>
    </aside>
  );
}
