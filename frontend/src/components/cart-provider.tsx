  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (Array.isArray(parsed)) setItems(parsed.map(normalize));
      }
    } catch {
      // Ignore invalid cart cache.
    }
    setReady(true);
  }, []);
  ...
  const value = useMemo<CartContextValue>(() => ({
    ...
    addItem: (item) => {
      ...
      setItems((current) => {
        ...
        const exists = current.find((entry) => keyFor(entry) === key);
        return exists ? current.map((entry) => keyFor(entry) === key ? { ...entry, quantity: entry.quantity + next.quantity } : entry) : [...current, next];
      });
      setStatus('idle');
      setCheckouts([]);
    },
        ...
        <div className="cartOverlay" role="dialog" aria-modal="true" aria-label="Cart">
          <aside className="cartDrawer">
            ...
            <form className="cartCheckoutForm" onSubmit={checkout}>
              ...
              <input value={deliveryLocation} onChange={(event) => setDeliveryLocation(event.target.value)} placeholder="Delivery location / landmark" />
              <textarea rows={2} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Order note" />
              <button type="submit" disabled={status === 'submitting' || items.length === 0}>{status === 'submitting' ? 'Creating checkout…' : 'Checkout with Paystack'}</button>
              {status === 'error' ? <p className="requestFeedback error">{error}</p> : null}
              {status === 'success' ? <div className="cartCheckoutLinks"><p className="requestFeedback success">Checkout ready. Pay below.</p>{checkouts.map((checkoutItem) => checkoutItem.checkoutUrl ? <a key={checkoutItem.reference} href={checkoutItem.checkoutUrl}>Pay {checkoutItem.merchantId}</a> : <p key={checkoutItem.reference}>Checkout URL unavailable for {checkoutItem.reference}</p>)}</div> : null}
            </form>
          </aside>
        </div>
      ) : null}
...
export function CartNavButton() {
  ...
  return <button type="button" className="cartNavButton" onClick={openCart}>🛒 Cart {itemCount ? `(${itemCount})` : ''}{subtotal > 0 ? ` · ${money(subtotal)}` : ''}</button>;
}
