...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const hasServiceItems = useMemo(() => items.some((item) => item.type === 'SERVICE'), [items]);
  ...
  async function checkout(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (hasServiceItems) {
        throw new Error('Services must be booked from the service page so you can choose a date and time.');
      }
